...
def get_decoder(code_size):
    ...
    decoder.add(L.Conv2DTranspose(filters=512, kernel_size=(3, 3), strides=(4, 4), activation='elu', padding='same'))
    decoder.add(L.Conv2DTranspose(filters=256, kernel_size=(3, 3), strides=2, activation='elu', padding='same'))
    decoder.add(L.Conv2DTranspose(filters=128, kernel_size=(3, 3), strides=2, activation='elu', padding='same'))
    decoder.add(L.Conv2DTranspose(filters=64, kernel_size=(3, 3), strides=(4, 2), activation='elu', padding='same'))
...
def get_model(input_shape=MODEL.input_shape, code_size=MODEL.code_size,
              ...
              use_dropout=True, print_summary=True,
              encoder_weights=None, decoder_weights=None):
    K.clear_session()
